...
2. Add the API Key to the .env file:
   - Go to the project folder where you intend to use the Telegram bot.
   - Open the .env file (usually represented by a gear icon).
   - Create a new line with the key as `TELEGRAM_TOKEN` and the value as the API token provided by BotFather.
"""
...
from core_functions import get_latest_data
...
def setup_routes(app, client, tool_data, assistant_id):
    ...
    def echo_all(telegram_chat_id, messageEN, chunk):
        ...
        #grab data from arduino
        latest_data = get_latest_data()
        latest_data_str = json.dumps(latest_data) if latest_data else " "
        

        prompt_with_context = f"User Input: {messageEN}\n\nContext from Documents: {context_docs}\n\n {latest_data_str}\n\nIMPORTANT: Provide a summarized response under 150 words follwoing the instructions provided to you as an assistant, and format this all for a text message, avoid backend language and keep it concise."
